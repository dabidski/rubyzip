...
class ZipFileTest < MiniTest::Test
  ...
  def test_create_from_scratch_with_old_create_parameter
    ...
    zf = ::RubyZip::File.new(EMPTY_FILENAME, 1)
    zf.get_output_stream('myFile') { |os| os.write 'myFile contains just this' }
    zf.mkdir('dir1')
    zf.comment = comment
    zf.close
  ...
  def test_get_output_stream
    ...
    ::RubyZip::File.open(TEST_ZIP.zip_name) do |zf|
      ...
      custom_entry_args = [TEST_COMMENT, TEST_EXTRA, TEST_COMPRESSED_SIZE, TEST_CRC, ::RubyZip::Entry::STORED, TEST_SIZE, TEST_TIME]
  ...
  def test_open_buffer_with_stringio
    ...
    ::RubyZip::File.open_buffer string_io do |zf|
  ...
  def test_open_buffer_without_block
    string_io = StringIO.new File.read('test/data/rubycode.zip')
    zf = ::RubyZip::File.open_buffer string_io
    assert zf.entries.map { |e| e.name }.include?('zippedruby1.rb')
  end
  ...
  def test_rename_entry_to_existing_entry
    ...
    zf = ::RubyZip::File.new(TEST_ZIP.zip_name)
    assert_raises(::RubyZip::EntryExistsError) { zf.rename(entry1, entry2) }
  ensure
    zf.close
  end
  ...
  def test_streaming
    ...
    File.open(zname, 'rb') do |f|
      RubyZip::File.open_buffer(f) do |zipfile|
  ...
  def test_nonexistant_zip
    assert_raises(::RubyZip::Error) do
      ::RubyZip::File.open('fake.zip')
    end
  end

  def test_empty_zip
    assert_raises(::RubyZip::Error) do
      ::RubyZip::File.open(TestFiles::NULL_FILE)
    end
  end
  ...
  def test_odd_extra_field
    ...
    File.open 'test/data/oddExtraField.zip', 'rb' do |zip_io|
      RubyZip::File.open_buffer zip_io.read do |zip|
  ...
  def test_open_xls_does_not_raise_type_error
    ::RubyZip::File.open('test/data/test.xls')
  end
